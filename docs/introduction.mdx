...
export const GeoJsonExample = ({ type, coordinates }) => {
  const example = {
    type: "Feature",
    geometry: {
      type,
...
|------|----------|-----------|
| Point | - single location in space | <GeoJsonExample type="Point" coordinates={[77.038, 28.728]}/>
...
### GeoJSON Layer Types
![vector raster types](../static/image/vector-raster-types.png)
```mermaid
graph TB
    subgraph "GIS Data Layers"
    A[Point Layer] --> B[Polygon Layer 1]
    B --> C[Polygon Layer 2]
